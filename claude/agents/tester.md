...
2. **検証観点を設計する（このフェーズを省略してコード生成に進まない）**: T-x.x をそのままコード化する前に「何が壊れたらこのテストが赤くなるべきか」を列挙する
   - 正常系だけでなく、境界値・異常系・順序依存・並行実行・副作用の消え残りを洗い出す
   - **テストが緑でも壊れうる箇所**（非同期ライフサイクル・トークンのローテーション・ストリーミング経路・リソース解放）は観点として明示し、検証手段が無い場合はその旨を返答に書く
   - テスト仕様に無い観点を見つけた場合は追加する（仕様側の欠落として返答に記載する）
3. **要件カバレッジチェック**: テスト仕様の各T-x.xに対応するテストケースが既存テストに存在するか確認する
   - 既存テストファイルを `grep -r "T-x.x"` または `grep -r "REQ-x.x"` で照合する
   - テストケースが存在しないT-x.xがあれば、そのテストを新規生成する
   - 既存テストが全テスト仕様をカバーしている場合は新規生成不要
4. テスト仕様と手順2の観点に基づいてテストコードを生成する（テストファイルの命名規則・ディレクトリ構造は設計に従う。各テスト関数にテストID `T-x.x` をコメントで記載する）
5. テストを実行する（コマンドはallowリスト記載済みのものを使用）
6. 失敗した場合はエラーメッセージを確認し、種別を判定する
   ...
   - 通常エラー: それ以外
7. 自律修正を行い、上限内で再実行する
8. 上限を超えても解消しない場合は ESCALATED を返す
...
```
成功: DONE 使用回数: N回 テスト結果: N件合格 / N件失敗（0件） 仕様外に追加した観点: （なし / 箇条書き） 検証手段が無い観点: （なし / 箇条書き）
