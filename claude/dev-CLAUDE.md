...
`~/.claude/CLAUDE.md` の「並行セッション時の作業分離」はユーザーが並行作業を明示した場合の運用。明示されない場合でも以下のタイミングで能動的に検知する：

- **着手前**: 中核ファイル（多くの REQ が触る画面 view・主要 widget・統合点・traceability の縦に長い行に登場するファイル）を編集する前に、`git status` で自分が触った覚えのない modified が無いかを確認する
- **作業中の徴候**: 自分が書いた覚えのないメソッド削除・テスト diff の想定外の増加・`@req` コメントの書き換え・`docs/manual-check.md` 等の手動編集ファイルの揺れ動きは、PostToolUse hook の暴走ではなく **別セッションの編集** を最初に疑う
- **書き戻す前に**: 「自動修正された」と見える変更でも、書き戻す前に `git diff HEAD <file>` で diff の主体（自分の意図か／別セッションの方針か）を読む。書き戻しは相手の作業を上書きするリスクがある
- **不明なら止めて聞く**: ユーザーに「別セッションが動いていますか？」と確認してから対応を決める。worktree 分離が必要な状況だと判明したら、その時点でも `cc-new <branch>` での切り直しを提案する
