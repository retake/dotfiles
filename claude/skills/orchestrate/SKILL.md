...
### ステップ0.5: 未完了アクションアイテムの確認（opt-in）
...
実行条件: `docs/requirements.md` + `src/` or `lib/` が存在する場合のみ。
実行時: Skill `sync` を呼び出し、乖離があれば「更新/スキップ/中止」をFR-9形式で確認停止する。
...
- 各FR開始時：フェーズ進捗の開始日時をdateコマンドで記録（再開時は上書きしない）
- 各FR完了時：ステータスをDONEに更新し完了日時を記録
- 確認停止時：status=waiting_approval、phase=停止フェーズ、current_decision に待機理由を記載
- サブエージェント完了後：「## エージェント報告」セクションを動的追加して追記

### 動的フィールドの追加タイミング

テンプレートは最小構成。以下のフィールドは該当FRの初回実行時に追記する：
- **FR-3初回**: FR-3設計レビュー試行回数: 0
- **FR-4初回**: FR-4再実行回数: 0（上限: 1）/ FR-4実装レビュー試行回数: 0
- **FR-5/6初回**: FR-5試行回数: 0 / FR-6試行回数: 0
- **FR-7初回**: FR-7完了レビュー試行回数: 0 / リスクレベル
- **エスカレーション初回**: 次のESC番号: 001 / 「## エスカレーション記録」セクション
...
- **waiting_approval**: status / phase / current_decision（待機理由）/ next_action（再開方法）を更新
- **escalated**: 上記に加え「## エスカレーション記録」に `ESC-NNN`（フェーズ・原因・対象ファイル・試行履歴・再開方法）を追記し、連番を+1する
...
`[DONE]` フォーマットで以下のセクションを出力する（task-state.md から算出）：

1. **成果物一覧** — 実装/テスト/traceability.md/completion-summary.md
2. **要件充足状況** — 要件総数・実装済み・テスト対応済み
3. **要確認事項** — completion-summary.md「人間が確認すべき点」を転記（なければ「なし」）
4. **リードタイム統計** — 総リードタイム + フェーズ別所要時間・ループ回数テーブル（確認停止の待機時間は除外）
5. **ツール呼び出し統計** — 総使用回数/上限 + エージェント別内訳
6. **fast track事後確認**（fast_trackモード時のみ）— スキップした確認停止一覧 + allowリスト追加提案
7. **Codexセカンドオピニオン推奨**（リスクhigh or セキュリティ/非同期変更時のみ）— 推奨理由 + 手順
8. `次のアクション: git diff / git add / git commit / git push（人間が実行）`
...
`~/retrospectives/_index.md` の各エントリの日付タグを確認し、90日以上更新されていない教訓を `[INFO]` で通知する（今回適用されたかどうかも併記）。
