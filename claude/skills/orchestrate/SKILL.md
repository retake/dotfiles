...
- `running` → 以下を表示して確認停止する（誤って2回起動した場合のstate破壊を防ぐ）：
  ...
  フェーズ: （phase）
  選択肢:
    A) 継続する場合: 「継続」と入力（task-state.mdのphaseフィールドが示すFRの先頭から再開する）
...
承認・修正・スキップいずれの場合も、次は「FR-1完了後のallowリスト確認停止」へ進む。
...
**初回Reviewer呼び出しはカウントしない。** DONE（自動修正1件以上）を受け取るたびにカウント+1・上限チェックを行い、上限以内であればFR-5→FR-6→Reviewerを再実行する。
重大な課題がなくなるまで繰り返す（上限: small=2 / medium=3 / large=5）。
