...
- タスクID、割り当て枠、要件（目的・完了条件・スコープ・制約）、技術スタック、過去の教訓
- 再実行時のみ: 前回の設計レビュー指摘
...
- タスクID、割り当て枠、設計（design-summary.md全文）、過去の教訓
...
- 検証方法（requirements.mdの該当セクション）、過去の教訓

**Linterへのプロンプト：** linter.mdの「入力プロンプトフォーマット」に従い、以下を渡す：
- タスクID、割り当て枠、タスクサイズ、自律修正の上限回数、過去の教訓
