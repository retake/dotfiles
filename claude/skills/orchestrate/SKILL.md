...
確定後は各フェーズの割り当て枠も確定サイズに合わせて再計算して用いること。
...
| FR-6 | lint | PENDING | - |
| FR-7 | レビュー | PENDING | - |
| FR-8 | 成果物出力 | PENDING | - | ※ ReviewerのDONEで同時にDONEとなる |
...
**初回Reviewer呼び出しはカウントしない。** 自動修正が発生するたびに試行回数をインクリメントする（= 2回目以降のReviewer呼び出し前にカウント+1・上限チェック）。
重大な課題がなくなるまで繰り返す（上限: small=2 / medium=3 / large=5）。
